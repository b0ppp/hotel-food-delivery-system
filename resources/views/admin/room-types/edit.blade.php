        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 sm:p-8 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.roomtypes.update', $roomType->id) }}" class="space-y-6">
                        @csrf
                        @method('PUT') {{-- Method untuk update adalah PUT atau PATCH --}}

                        <div>
                            <x-input-label for="name" :value="__('Nama Tipe Kamar:')" />
                            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $roomType->name)" required autofocus placeholder="Contoh: Standard, Deluxe" />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        {{-- Tidak ada field deskripsi lagi --}}
