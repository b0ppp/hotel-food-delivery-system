<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-4xl text-black leading-tight">
            {{ __('Edit Item Menu: ') }} {{ $menuItem->item_name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 sm:p-8 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.menuitems.update', $menuItem->menu_item_id) }}" class="space-y-6">
                        @csrf
                        @method('PUT') {{-- Method untuk update adalah PUT atau PATCH --}}

                        <div>
                            <x-input-label for="item_name" :value="__('Nama Item Menu:')" />
                            <x-text-input id="item_name" class="block mt-1 w-full" type="text" name="item_name" :value="old('item_name', $menuItem->item_name)" required autofocus />
                        ...
                        <div>
                            ...
                            <select id="availability_status" name="availability_status" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                <option value="Tersedia" {{ old('availability_status', $menuItem->availability_status) == 'Tersedia' ? 'selected' : '' }}>
                                    Tersedia
                                </option>
                                <option value="Tidak Tersedia" {{ old('availability_status', $menuItem->availability_status) == 'Tidak Tersedia' ? 'selected' : '' }}>
                                    Tidak Tersedia
                                </option>
                            </select>
