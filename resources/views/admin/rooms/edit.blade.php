        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 sm:p-8 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.rooms.update', $room->room_id) }}" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="room_number" :value="__('Nomor Kamar:')" />
                            <x-text-input id="room_number" class="block mt-1 w-full" type="text" name="room_number" :value="old('room_number', $room->room_number)" required autofocus placeholder="Contoh: 101, 205A, VIP-01" />
                        ...
                        <div>
                            ...
                            <select id="room_type_id" name="room_type_id" {{-- UBAH name menjadi room_type_id --}}
                                ...
                                @foreach ($roomTypes as $type)
                                    ...
                                    {{--  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                                        Gunakan $room->room_type_id untuk pre-select.
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                @foreach ($statuses as $statusOption) {{-- $statuses dikirim dari controller edit() --}}
                                    <option value="{{ $statusOption }}" {{ old('status', $room->status) == $statusOption ? 'selected' : '' }}>
                                        {{ $statusOption }}
                                    </option>
                                @endforeach
                            </select>
